<template>
      ...
      <div class="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-lg w-full max-w-md">
        ...
        <form v-else @submit.prevent="handleSignup">
          ...
          <div class="mb-6 relative">
           ...
           <!-- Role Selection (Optional - Defaulting to 'staff' as per API) -->
          <!-- You could add a dropdown here if needed, but API defaults to 'staff' -->
          <!-- <div class="mb-6">
            <label for="signup-role" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Role</label>
            <select v-model="signupForm.role" id="signup-role" class="w-full p-2 border rounded-md dark:bg-gray-700 dark:text-white dark:border-gray-600">
              <option value="staff">Staff</option>
              <option value="dentist">Dentist</option> // Assuming 'dentist' is another role
            </select>
          </div> -->
  ...
  import { Stethoscope, User, Lock, LoaderCircle, X } from 'lucide-vue-next';
  ...
  const setAccessTokenCookie = (token) => {
    Cookies.set('dental_access_token', token, { expires: 1, path: '/' }); // Expires in 1 day
  };
  ...
  const handleLogin = async () => {
    ...
    error.value = null;
    try {
      const response = await axios.post(`${config.public.API_BASE_URL}/dental/auth/login`, {
  ...
  onMounted(() => {
    const existingToken = getAccessTokenCookie();
    if (existingToken) {
